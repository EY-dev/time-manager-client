<template>
    <div class="forgot-password elevation-15">
        <h1> {{ title }} </h1>
        <h2> {{ subTitle }} </h2>
        <form class="forgot-password-form" @submit.prevent="null">
            <div class="field">
                <label for="email"> {{ $store.getters.getDictionary.forgotPassword.email }} </label>
            ...
            <div class="field">
                <label for="code"> {{ $store.getters.getDictionary.forgotPassword.code }} </label>
                <input type="text" id="code" name="user_code" class="input-field" v-model="form.code">
                <div class="text-error"> <span>{{ errors.code }}</span><v-icon class="i-error">error_outline</v-icon></div>
                <div class="text-succeed"><span>{{ succeed.code }}</span><v-icon class="i-succeed">check</v-icon></div>
            </div>
            <div class="field">
                <label for="pwd"> {{ $store.getters.getDictionary.forgotPassword.password }} </label>
            ...
            <div class="field">
                <label for="confPwd"> {{ $store.getters.getDictionary.forgotPassword.confPassword }} </label>
            ...
            <div class="action-box">
                ...
                <v-btn class="action-btn" rounded @click="sendData"> {{ btnTitle }} </v-btn>
                <div class="field signup-link" style="opacity: 1">
                    <router-link to="/login" class="to-signin"> {{ $store.getters.getDictionary.forgotPassword.signin }} </router-link>
                </div>
            </div>
        </form>
...
export default {
    ...
    components: {Loading},
    data:()=>({
        form : {code: '', email:'', pwd: '', confPwd: ''},
        errors : {code: '', email:'', pwd: '', confPwd: ''},
        succeed : {code: '', email:'', pwd: '', confPwd: ''},
        title : null,
        subTitle: null,
        blocks: null,
        btnTitle: null,
    ...
    methods:{
        ...
        async sendData(){
            if (this.currentStep === 0){
                if (this.validEmail()){
                    document.querySelector(".forgot-password-form").classList.add("blur-10");
                    await delay(250);
                    this.isLoading = true;
                    await delay(1000);//get result
                    this.isLoading = false;
                    document.querySelector(".forgot-password-form").classList.remove("blur-10");
                    await delay(750);
            ...
            if (this.currentStep >= 2){
                if (this.validPwd() && this.validConfPwd()){
                    document.querySelector(".forgot-password-form").classList.add("blur-10");
                    await delay(250);
                    this.isLoading = true;
                    await delay(1000);//get result
        ...
        validate(){
            let status = true
            if (!this.validCode()){status = false}
        ...
        nextStep(){
            this.title = this.$store.getters.getDictionary.forgotPassword.title[(this.currentStep+1)][0];
            this.subTitle = this.$store.getters.getDictionary.forgotPassword.title[this.currentStep+1][1];
            this.blocks = document.querySelectorAll('.field');
            this.blocks.forEach(block => {
                block.classList.remove("show");
            })
            this.blocks[this.currentStep+1].classList.add("show");
            this.btnTitle = this.$store.getters.getDictionary.forgotPassword.actionBtn[this.currentStep+1];
            if (this.currentStep === 1){
                this.blocks[this.currentStep+2].classList.add("show");
            }
            if (this.currentStep === 2){
                this.currentStep = -1;
            }
        ...
        validCode(){
            if (this.form.code === ""){
                this.setError(this.$store.getters.getDictionary.errors.emptyField, "code");
                return false;
            }
            return true;
        },
    ...
    mounted() {
        ...
        document.querySelector('.to-signin').addEventListener("click", () => {
            document.querySelector('.forgot-password').classList.add("move-out-blur");
        })
